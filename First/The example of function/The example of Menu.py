...
__author__ = 'Hongten'
MENU_ITEMS = ['文件', '编辑', '格式', '运行', '设置', '窗口', '帮助']
# 菜单File中的选项
...
# 加载菜单配置,没有这个命令不能加载到页面
root.config(menu=menubar)

mainloop()
# 4月10日
# 第二个例子
# 使用tkinter开发一个 搜图王的小界面程序  这里使用的不是创建方法然后调用,而是直接在主窗体中添加菜单栏跟控件
from tkinter import *
import tkinter.messagebox as messagebox

root = Tk()  # 创建一个主窗口
root.title("搜图助手")  # 为主窗口命名
root.geometry("500x500+600+200")  # 设置主窗体大小
help = messagebox.showinfo("欢迎", "欢迎使用搜图助手！\nBy Johnson")  # 定义一个弹出的消息框
...
menubar = Menu(root)  # 创建一个菜单控件

filemenu = Menu(menubar, tearoff=0)  # 创建菜单控件中的文件菜单

menubar.add_cascade(label="菜单", menu=filemenu)  # 添加子选项 为菜单控件中的文件菜单命名

filemenu.add_command(label="上一张", command=hello)  # 添加命令,label参数为显示内容
filemenu.add_command(label="下一张", command=hello)
...
helpmenu = Menu(menubar, tearoff=0)  # 创建菜单控件中的子菜单  帮助菜单

menubar.add_cascade(label="帮助", menu=helpmenu)  # 为帮助菜单命名

helpmenu.add_command(label="关于", command=hello)  # 添加命令
helpmenu.add_separator()  # 添加分割线
helpmenu.add_command(label="使用方法", command=hello)  # 添加命令

root.config(menu=menubar)  # 将菜单加入到主窗体中,配置
root.mainloop()
